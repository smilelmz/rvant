...
const CollapseIcon = (
  <svg className={bem('collapse-icon')} viewBox='0 0 30 24'>
    <path
...
const DeleteIcon = (
  <svg className={bem('delete-icon')} viewBox='0 0 32 22'>
    <path
...
const NumberKeyboardKey = (fieldProps: NumberKeyboardKeyProps) => {
  const props: NumberKeyboardKeyProps = {
    style: {},
    className: '',
    ...fieldProps
  }
  ...
  const onTouchEnd = (event: TouchEvent) => {
    if (active.current) {
      ...
      active.current = false
      props.press && props.press(props.text, props.type)
    }
  }
  ...
  return (
    <div
      style={style}
      className={`${bem('wrapper', { wider: props.wider })} ${className || ''}`}
...
export default React.memo(NumberKeyboardKey)
